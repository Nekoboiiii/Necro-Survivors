...
public class PlayerInventory : MonoBehaviour
{
    ...
    public class Slot
    {
        ...
        public List<PassiveData> availablePassives = new List<PassiveData>(); // List if upgrade options for passiv items
        ...
        // Find a weapon of a certain type in th inventory
        ...
        {
            // Remove this weapon from the uprade pool
        ...
        {
            // Remove this passive from the uprade pool
            if(removeUpgradeAvailability)
            {
                availableWeapons.Remove(data);
            }
        ...
        // We also have an optional boolean to remove this item drom the upgrade list
        ...
        // returns the slot number that the item was put inn
        public int Add(WeaponData data)
        {
            int slotNum = -1;

            // Try tto find an empty Slot
        ...
        // Finds an empty slot and adds a passive of acertain type 
        ...
        public int Add(PassiveData data)
        {
            ...
            // Otherwise create the passiv in the slot
        ...
        public void LevelUpPassiveItem(int slotIndex, int upgradeIndex)
        {
            if (passiveSlots.Count > slotIndex)
            {
                ...
                // Dont level up the passive item if it is already at max level
                if (!p.DoLevelUp())
                {
        ...
        void ApplyUpgradeOptions()
        {
            ...
            foreach (UpgradeUI upgradeOption in upgradeUIOptions)
            {
                ...
                if (upgradeType == 1)
                {
                    // Pick a weapon upgrade, then remove it so that we dont get it twice
                    ...
                    if (chosenWeaponUpgrade != null)
                    {
                        ...
                        for (int i = 0; i < weaponSlots.Count; i++)
                        {
                            ...
                            if (w != null && w.data == chosenWeaponUpgrade)
                            {
                                ...
                                upgradeOption.upgradeButton.onClick.AddListener(() => LevelUpWeapon(i, i)); // Apply button functionanlity
                    ...
                    if (chosenPassivUpgrade != null)
                    {
                        ...
                        for (int i = 0; i < passiveSlots.Count; i++)
                        {
                            ...
                            if (p != null && p.data == chosenPassivUpgrade)
                            {
                                ...
                                // Set the Event Listener item and level description to be that of the next level
                                upgradeOption.upgradeButton.onClick.AddListener(() => LevelUpPassiveItem(i, i)); // Apply button functionanlity
    ...
    void RemoveUpgradeOptions()
    {
        foreach (UpgradeUI upgradeOption in upgradeUIOptions)
        {
            ...
            DisableUpgradeUI(upgradeOption); // Call the DisableUpgradeUI method here to disavle all UI options applying upgrades to them
        }
    }
